...
const ActionLink: React.FunctionComponent<LinkProps> = (props) => {
  ...
  return (
    <Layout bankAccount={bankAccount} >
      ...
      <div>
        ...
        <table className={`table tale-bordeless table-striped ${classes.tableTransactions}`}>
          <thead>
            <tr>
              <th scope="col">Data</th>
              <th scope="col" colSpan={3}>Descrição</th>
              <th scope="col" className="text-right">Valor (R$)</th>
            </tr>
          </thead>
          <tbody>
            {transactions.map((t, key) => (
              <tr key={key}>
                <td>
                  {format(parseISO(t.created_at), 'dd/MM')}
                </td>
